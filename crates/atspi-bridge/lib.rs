...
pub async fn window_events_test() -> Result<(), Box<dyn Error>> {
	...
	// FIXME: currently investigating an issue where vscode windows
	// inconsistently have active set
	// might be resolved by having ever registered for window events
	// during lifetime of that vscode, requiring a workaround.
	...
	for child in root.get_children().await? {
		...
		if let Ok(application) = proxy.get_application().await {
			eprintln!("application: {application:?} {}", application.name);
